...
`editor-view` owns the Obsidian custom Markdown view. It routes Markdown leaves into Libre Note Editor, manages file lifecycle, loads rich HTML, wires autosave and conflict handling, refreshes link warnings, coordinates desktop ODT sync, and renders the React app.
...
- `interfaces.ts` defines editor view options, render target state, loaded state, source targets, desktop source targets, and registration contracts.
- `constants.ts` defines view types, command ids/names, and supported Markdown extension values.
...
- `desktop-source/desktopSource.ts` flushes pending saves, syncs saved ODT changes, and prepares ODT state after load.
- `link-warnings/linkWarnings.ts` refreshes open editor views when Obsidian metadata changes.
- `options/options.ts` builds production `EditorViewOptions` from Obsidian app, settings, rich-document store, conversion, link resolver, and source-write modules.
...
`main.ts` registers this view and routes Markdown leaves to it. `EditorView` then uses `rich-html`, `markdown-sync`, `source-write`, `conversion`, `conflicts`, `settings`, and `obsidian-links` through the option callbacks created in `options/options.ts`.
